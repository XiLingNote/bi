		...
		<div class="modal-dialog">
			<div class="modal-content message_align">
				...
				<div class="modal-body">
					...
					<span class="deleteRows"></span>
				</div>
				<div class="modal-footer">
					<input type="hidden" id="url" />
					<button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
					<a onclick="deleteByDate()" class="btn btn-success"
...
<script src="<%=basePath%>/js/jquery.form-3.51.0.js"></script>
...
<script src="<%=basePath%>/js/bootstart-table-knockout.js"></script>
...
<script>
     ...
     var    tableTitleName="WirelessFlowStructure";
        var role=<%=sub.isPermitted("all")||sub.isPermitted("delete_history")%>;
    	
        $(function() {
        	$("#importData").attr("class", "nav nav-second-level in"); 
        	$("#leftFlowStructure").attr("class", "nav nav-third-level in");
      	  $("#leftwirless").css("background","#578EBE");
      	...
      	//初始化表格
        	...
        	operate.operateInit();
        	//初始化上传文件
             inportInit("txt_file", "<%=basePath%>wirelessFlowStructure/uploadWirelessFlowStructure.do");
    	//	pandora潘多拉官方旗舰店
        ...
        function initTable(tableTitle) {
            ...
            $("#cusTable").bootstrapTable({
                ...
                url: "<%=basePath%>wirelessFlowStructure/queryWirelessFlowStructureByPage.do", //获取数据的Servlet地址
                striped: true,  //表格显示条纹
                ...
                onEditableSave: function (field, row, oldValue, $el) {
                    $.ajax({
                        type: "post",
                        url: "<%=basePath%>wirelessFlowStructure/updateWirelessFlowStructure.do",
                        data: row,
                        dataType: 'JSON',
                ...
                queryParams: function queryParams(param) {   //设置查询参数
                     param = {
                        ...
                        sortName:this.sortName?this.sortName:"date",
            ...
            $('#cusTable').bootstrapTable('hideColumn', 'flowId');
    ...
    //初始化fileinput
      ...
      function deleteByArray(){
    	...
    	var a=  {"idArray":idArray};
          $.ajax({
              url: "<%=basePath%>wirelessFlowStructure/deleteWirelessFlowStructureByBatch.do",
              type: "post",
              data:a,
              success: function (data, status) {
					toastr.success(JSON.parse(data).data);

            	  $('#cusTable').bootstrapTable('remove', {field: 'flowId', values: idArray})
                	if($('#cusTable').bootstrapTable('getData').length==0)
                    	getTitle(tableTitleName,role);
              }
          });
      ...
      function deleteByDate(){
    	  a={"date":$("#queryStartDate").val()};
    	  $.ajax({
              url: 
            	  "<%=basePath%>wirelessFlowStructure/deleteWirelessFlowStructure.do",
					type : "post",
					data : a,
					success : function(data, status) {
						toastr.success(JSON.parse(data).data)
