...
<script type="text/javascript">
	...
	/* 如果需要查询按钮触发查询效果，所有change事件中不用调用本方法 */
	function getDate() {
		/* beginStr，endStr：yyyy/mm/dd */
		alert("dateType:" + dateType + ",beginStr:" + beginStr + ",endStr:"
				+ endStr);
		/* ajax后台请求 */
	...
	$(function() {
		/* 年日期选项 */
		dateOption.minView = 4;
		dateOption.startView = 4;
		dateOption.format = "yyyy";
		$('#s1>input[name = begin ]').datetimepicker(dateOption);
		$('#s1>input[name = end ]').datetimepicker(dateOption);
		/* 月日期选项 */
		dateOption.minView = 3;
		dateOption.startView = 3;
		dateOption.format = "yyyy/MM";
		$('#s2>input[name = begin ]').datetimepicker(dateOption);
		$('#s2>input[name = end ]').datetimepicker(dateOption);
		/* 日日期选项 */
