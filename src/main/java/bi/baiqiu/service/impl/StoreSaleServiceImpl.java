...
public class StoreSaleServiceImpl implements StoreSaleService {
	...
	public Map<String, Object> queryThisMonth(String amountType,
		...
		// 所有部门
		Set<String> departments = jedis.smembers(KeyUtils.departments);
		for (String depart : departments) {
			// 店铺集合
			Set<String> stores = jedis.smembers(depart + KeyUtils.EN);
			storeShorts.addAll(stores);
		}

		if (StringUtils.isNotBlank(department)) {
			if (department.equals(KeyUtils.All)) {
				querySet = storeShorts;
			} else {
				querySet = jedis.smembers(department + KeyUtils.EN);
			}
		} else {
			return null;
		}
...
//		String yyyyMM = "201705";
		...
		for (String shopName : querySet) {
			...
			String dataString = jedis.hget(shopName + KeyUtils.MONTH, yyyyMM);
		...
		for (String shopName : querySet) {
			...
			for (int i = 1; i <= 31; i++) {
				...
				String dataString = jedis.hget(shopName + KeyUtils.DAY,
						yyyyMMdd);
				if (StringUtils.isNotBlank(dataString)) {
	...
	public Map<String, Object> queryByThisDay(String dateType, Date date,
		...
		if (KeyUtils.YEAR.equals(dateType)) {
		...
		for (String department : departments) {
			Set<String> store = jedis.smembers(department + KeyUtils.EN);
			set.addAll(store);
		}
	...
	public Set<String> queryStoreName() {
		Jedis jedis = jedisPool.getResource();
		Set<String> departNameSet = jedis.smembers(KeyUtils.departments);
	...
	public Map<String, Object> storeSalesDemonstrate(String storeName) {
		...
		String dataString = jedis.hget(storeName + KeyUtils.MONTH, "201705");
		if (StringUtils.isNotBlank(dataString)) {
			RedisPojo redisPojo = GsonUtils.gson.fromJson(dataString,
					RedisPojo.class);
		...
		String Yesterday_dataString = jedis.hget(storeName + KeyUtils.DAY, Yesterday);
	...
	public Map<String, Object> getRecentlyDateAndGmvlist(String storeName) {
		...
		for (String date : recentlyDate) {
			dataString = jedis.hget(storeName + KeyUtils.DAY, date);
			gmv = 0;
			if (StringUtils.isNotBlank(dataString)) {
				RedisPojo redisPojo = GsonUtils.gson.fromJson(dataString,
						RedisPojo.class);
