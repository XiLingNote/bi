...
public class JdGetDataTask {
 ...
 //@Scheduled(fixedDelay = 30 * 1000)
	...
	public void getDataBystartDate(String startDate, Date taskStartDate) throws JdException, ParseException {
		...
		for (ShopBean shop : list) {
			...
			do {
				...
				nextEndCalendar.setTime(
						nextEndCalendar.compareTo(calendar) < 0 ? nextEndCalendar.getTime() : calendar.getTime());
				...
				log.info("拉取间隔时间：" + DateUtils.dateToString(nextStartCalendar.getTime()) + "到"
						+ DateUtils.dateToString(nextEndCalendar.getTime()));
	 ...
	 */
	public List<OrderSearchInfo> getJdDataByClientAndRequestBase(JdClient client, OrderSearchRequest request)
			throws JdException {
